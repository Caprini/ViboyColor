...
# -*- coding: utf-8 -*-
"""
Test Step 0480: Verificar que seleccionar fila de botones afecta el nibble bajo.

Según Pan Docs, escribir JOYP = 0x20 (P15=1, seleccionar fila de botones) debe
hacer que los bits 0-3 reflejen el estado de los botones de acción.
...
def test_joyp_select_buttons_affects_low_nibble():
    """
    Verifica que seleccionar fila de botones afecta el nibble bajo.
    ...
    # Paso 1: Escribir JOYP = 0x10 (seleccionar fila de botones, P15=0, P14=1)
    # 0x10 = 0b00010000 (bit 5=0, bit 4=1, bits 6-7=0 pero se ponen a 1 en read)
    # bit 5=0 selecciona fila de acción
    mmu.write(0xFF00, 0x10)
    
    # Paso 2: Leer JOYP
    joyp_value = mmu.read(0xFF00)
    
    # Paso 3: Verificar que bits 0-3 reflejan estado de botones
    ...
    # Paso 5: Presionar botón A (índice 4) y verificar que bit 0 cambia a 0
    joypad.press_button(4)  # Botón A
    joyp_value_pressed = mmu.read(0xFF00)
    bit_0 = joyp_value_pressed & 0x01
    assert bit_0 == 0x00, (
        f"Bit 0 debe ser 0 (presionado) cuando se presiona A, "
        f"pero se leyó como {bit_0}. Valor completo: 0x{joyp_value_pressed:02X}"
    )
    
    # Paso 6: Soltar botón A y verificar que bit 0 vuelve a 1
    joypad.release_button(4)
    joyp_value_released = mmu.read(0xFF00)
    bit_0 = joyp_value_released & 0x01
    assert bit_0 == 0x01, (
        f"Bit 0 debe ser 1 (suelto) cuando se suelta A, "
