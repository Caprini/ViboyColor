...
PPU::PPU(MMU* mmu) 
    ...
    , frame_id_(0)  // Step 0497: Inicializar frame_id único
    , framebuffer_frame_id_(0)  // Step 0497: Inicializar frame_id del buffer front
...
void PPU::step(int cpu_cycles) {
    ...
    while (clock_ >= CYCLES_PER_SCANLINE) {
        ...
        if (ly_ > 153) {
            ...
            frame_counter_++;
            // --- Step 0497: Incrementar frame_id único ---
            frame_id_++;
...
uint64_t PPU::get_frame_id() const {
    return frame_id_;
}

uint64_t PPU::get_framebuffer_frame_id() const {
    return framebuffer_frame_id_;
}
...
uint8_t* PPU::get_framebuffer_rgb_ptr() {
    // --- Step 0455: Present automático si hay swap pendiente ---
    // Si hay contenido renderizado en el back buffer que no se ha presentado,
    // hacemos el swap automáticamente para que los tests (y el emulador) vean el contenido actualizado
    if (framebuffer_swap_pending_) {
        swap_framebuffers();
...
void PPU::swap_framebuffers() {
    ...
    // --- Step 0498: BufferTrace CRC - Capturar CRCs ANTES del swap ---
    ...
    // --- Step 0497: Asociar frame_id al buffer front ---
    framebuffer_frame_id_ = frame_id_;
    // -------------------------------------------
    
    // --- Step 0498: BufferTrace CRC - Capturar CRCs DESPUÉS del swap ---
    ...
    // NOTA: Ya se llama arriba para calcular CRC32, no duplicar
    // --- Step 0455: Debug - Verificar antes de convertir ---
    static int swap_debug_count = 0;
    if (swap_debug_count < 5) {
        swap_debug_count++;
        int front_non_zero = 0;
        int back_non_zero = 0;
        for (size_t i = 0; i < framebuffer_front_.size() && i < 100; i++) {
            if (framebuffer_front_[i] != 0) front_non_zero++;
            if (framebuffer_back_[i] != 0) back_non_zero++;
        }
        printf("[SWAP-DEBUG] Call #%d | Before convert: front_non_zero=%d/100, back_non_zero=%d/100\n",
               swap_debug_count, front_non_zero, back_non_zero);
    }
    // -------------------------------------------
    convert_framebuffer_to_rgb();
    // -------------------------------------------
...
void PPU::render_scanline() {
    ...
    if (ly_ == 0 && (tilemap_diag_count < 5 || (frame_counter_ % 120 == 0))) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            if (tile_map_addr >= 0x8000 && tile_map_addr <= 0x9FFF) {
                tile_ids_sample[i] = mmu_->read_vram(tile_map_addr);
            } else {
                ...
                if (tile_byte1 != 0x00 || tile_byte2 != 0x00) {
                    ...
                    if (signed_addressing) {
                        ...
                        for (int i = 0; i < 32; i++) {
                            ...
                            if (tile_map_addr >= 0x8000 && tile_map_addr <= 0x9FFF) {
                                tile_id_check = mmu_->read_vram(tile_map_addr);
                            }
    ...
    if (ly_ == 72 && tilemap_dump_count < 3) {
        ...
        for(int x_dump = 0; x_dump < 32; x_dump++) {
            ...
            if (tile_map_addr_dump >= 0x8000 && tile_map_addr_dump <= 0x9FFF) {
                tile_id_dump = mmu_->read_vram(tile_map_addr_dump);
            }
    ...
    for (int x = 0; x < 160; ++x) {
        ...
        if (vram_has_tiles_ && ly_ == 0 && tile_addr_verify_count < 3 && x == 0) {
            ...
            if (tile_map_base >= 0x8000 && tile_map_base <= 0x9FFF) {
                sample_tile_id = mmu_->read_vram(tile_map_base);
            }
        ...
        if (!tile_addr_valid || !tile_line_addr_valid) {
            ...
            // --- Step 0455: Escribir índice crudo (no aplicar BGP aquí) ---
            // El framebuffer debe contener índices crudos (0-3), no shades
            // La paleta BGP se aplicará en convert_framebuffer_to_rgb()
            ...
            // Escribir índice crudo (0-3), no shade
            framebuffer_back_[line_start_index + x] = color_index;
        } else {
            ...
            // --- Step 0455: Escribir índice crudo (no aplicar BGP aquí) ---
            // La paleta BGP se aplicará en convert_framebuffer_to_rgb()
            uint8_t final_color = color_index;  // Índice crudo, no shade
    ...
    // --- Step 0497: Corregir log para separar FRONT (presentado) y BACK (en construcción) ---
    ...
    if (ly_ == 0 || ly_ == 72 || ly_ == 143 || (ly_ % 20 == 0 && framebuffer_line_check_count < 20)) {
        ...
        // --- Step 0497: Log del buffer FRONT (el que se va a presentar) ---
        int index_counts_front[4] = {0, 0, 0, 0};
        int non_zero_pixels_front = 0;
        
        for (int x = 0; x < SCREEN_WIDTH; x++) {
            uint8_t color_idx = framebuffer_front_[line_start + x] & 0x03;
            if (color_idx < 4) {
                index_counts_front[color_idx]++;
                if (color_idx != 0) {
                    non_zero_pixels_front++;
                }
            }
        }
        
        printf("[PPU-FRAMEBUFFER-LINE-FRONT] Frame %llu | LY: %d | Non-zero pixels: %d/160 | "
               "Distribution: 0=%d 1=%d 2=%d 3=%d | frame_id=%llu\n",
               static_cast<unsigned long long>(frame_counter_ + 1),
               ly_, non_zero_pixels_front,
               index_counts_front[0], index_counts_front[1], index_counts_front[2], index_counts_front[3],
               static_cast<unsigned long long>(framebuffer_frame_id_));
        
        // --- Step 0497: Log del buffer BACK (en construcción) ---
        int index_counts_back[4] = {0, 0, 0, 0};
        int non_zero_pixels_back = 0;
        
        for (int x = 0; x < SCREEN_WIDTH; x++) {
            uint8_t color_idx = framebuffer_back_[line_start + x] & 0x03;
            if (color_idx < 4) {
                index_counts_back[color_idx]++;
                if (color_idx != 0) {
                    non_zero_pixels_back++;
                }
            }
        }
        
        printf("[PPU-FRAMEBUFFER-LINE-BACK] Frame %llu | LY: %d | Non-zero pixels: %d/160 | "
               "Distribution: 0=%d 1=%d 2=%d 3=%d | frame_id=%llu\n",
               static_cast<unsigned long long>(frame_counter_ + 1),
               ly_, non_zero_pixels_back,
               index_counts_back[0], index_counts_back[1], index_counts_back[2], index_counts_back[3],
               static_cast<unsigned long long>(frame_id_));
        
        // Verificar algunos píxeles específicos del FRONT
        int test_x_positions[] = {0, 40, 80, 120, 159};
        for (int i = 0; i < 5; i++) {
            int x = test_x_positions[i];
            uint8_t color_idx = framebuffer_front_[line_start + x] & 0x03;
            printf("[PPU-FRAMEBUFFER-LINE-FRONT] Pixel (%d, %d): index=%d\n", x, ly_, color_idx);
        }
        
        // Advertencia si la línea está completamente vacía (en FRONT)
        if (non_zero_pixels_front == 0 && ly_ < 144) {
            printf("[PPU-FRAMEBUFFER-LINE-FRONT] ⚠️ ADVERTENCIA: Línea %d completamente vacía en FRONT después de renderizar!\n", ly_);
        }
    }
    // -------------------------------------------
...
// --- Step 0455: Helper para aplicar paleta DMG ---
// Aplica registro de paleta (BGP/OBP0/OBP1) a color_idx para obtener shade
inline uint8_t dmg_apply_palette(uint8_t color_idx, uint8_t pal_reg) {
    // color_idx: 0..3
    // pal_reg: BGP/OBP0/OBP1 (0xFF47/0xFF48/0xFF49)
    // Cada índice ocupa 2 bits en el registro:
    //   bits 1-0: shade para color_idx 0
    //   bits 3-2: shade para color_idx 1
    //   bits 5-4: shade para color_idx 2
    //   bits 7-6: shade para color_idx 3
    return (pal_reg >> (color_idx * 2)) & 0x03; // shade 0..3
}

// Mapea shade DMG (0..3) a RGB888 (gris)
// Fuente: Pan Docs - "Color Palettes"
// Shade 0 = White (255, 255, 255)
// Shade 1 = Light gray (170, 170, 170)
// Shade 2 = Dark gray (85, 85, 85)
// Shade 3 = Black (0, 0, 0)
inline void dmg_shade_to_rgb(uint8_t shade, uint8_t& r, uint8_t& g, uint8_t& b) {
    static const uint8_t shade_table[4] = {255, 170, 85, 0};
    uint8_t gray = shade_table[shade];
    r = gray;
    g = gray;
    b = gray;
}
...
void PPU::convert_framebuffer_to_rgb() {
    // --- Step 0455: Fix - Aplicar paleta DMG correctamente ---
    // Convierte framebuffer_front_ (índices 0..3) a RGB888 usando BGP/OBP
    
    // --- Step 0455: Debug - Verificar entrada ---
    static int convert_entry_count = 0;
    if (convert_entry_count < 5) {
        convert_entry_count++;
        printf("[CONVERT-ENTRY] convert_framebuffer_to_rgb() called #%d\n", convert_entry_count);
    }
    // -------------------------------------------
    
    if (mmu_ == nullptr) {
        printf("[CONVERT-ERROR] mmu_ is nullptr!\n");
        return;
    }
    ...
    if (is_dmg) {
        ...
        // Aplicar paleta BGP para obtener shade, luego mapear shade→RGB888
        // Leer registros de paleta una vez
        ...
        if (convert_debug_count < 5) {
            ...
            int index_counts[4] = {0, 0, 0, 0};
            for (size_t i = 0; i < framebuffer_front_.size() && i < 100; i++) {
                uint8_t idx = framebuffer_front_[i] & 0x03;
                index_counts[idx]++;
                if (idx != 0) non_zero_count++;
            }
            printf("[CONVERT-DEBUG] Call #%d | First 100 pixels: non_zero=%d | Index distribution: 0=%d 1=%d 2=%d 3=%d | BGP=0x%02X\n",
                   convert_debug_count, non_zero_count, index_counts[0], index_counts[1], index_counts[2], index_counts[3], bgp);
        }
        // -------------------------------------------
        ...
        for (uint16_t y = 0; y < SCREEN_HEIGHT; y++) {
            for (uint16_t x = 0; x < SCREEN_WIDTH; x++) {
                ...
                // El framebuffer contiene índices crudos (0-3)
                uint8_t color_idx = framebuffer_front_[fb_index] & 0x03;
                
                // Aplicar BGP para obtener shade
                uint8_t shade = dmg_apply_palette(color_idx, bgp);
                
                // Convertir shade a RGB888 (gris DMG)
                uint8_t r, g, b;
                dmg_shade_to_rgb(shade, r, g, b);
                
                // Escribir RGB en framebuffer_rgb_front_
        ...
        // --- Step 0455: Debug - Verificar RGB escrito ---
        if (convert_debug_count <= 5) {
            printf("[CONVERT-DEBUG] RGB[0:10]: ");
            for (int i = 0; i < 10; i++) {
                printf("%d ", framebuffer_rgb_front_[i]);
            }
            printf("\n");
        }
        // -------------------------------------------
    } else {
        // --- Modo CGB: usar paletas CGB (código original) ---
...
// --- Step 0498: Tabla CRC32 estándar (polinomio 0xEDB88320) ---
...
uint32_t PPU::compute_crc32_full(const std::vector<uint8_t>& data, size_t size) const {
    ...
    size_t actual_size = (size < data.size()) ? size : data.size();
...
uint32_t PPU::compute_buffer_uid(const std::vector<uint8_t>& data) const {
    // Hash simple del contenido (primeros 100 bytes)
    uint32_t hash = 0;
    size_t sample_size = (data.size() < 100) ? data.size() : 100;
...
std::vector<BufferTraceEvent> PPU::get_buffer_trace_ring(size_t max_events) const {
    std::vector<BufferTraceEvent> events;
    
    if (max_events == 0 || max_events > BUFFER_TRACE_RING_SIZE) {
        max_events = BUFFER_TRACE_RING_SIZE;
    }
    
    // Obtener los últimos N eventos del ring buffer
    // Si el ring buffer no está lleno, empezar desde 0
    size_t available_events = (buffer_trace_ring_head_ < BUFFER_TRACE_RING_SIZE) 
                              ? buffer_trace_ring_head_ 
                              : BUFFER_TRACE_RING_SIZE;
    
    size_t events_to_return = (max_events < available_events) ? max_events : available_events;
    size_t start_idx = (buffer_trace_ring_head_ >= events_to_return) 
                       ? (buffer_trace_ring_head_ - events_to_return) 
                       : 0;
    
    for (size_t i = 0; i < events_to_return; i++) {
        size_t idx = (start_idx + i) % BUFFER_TRACE_RING_SIZE;
        events.push_back(buffer_trace_ring_[idx]);
    }
